...
<main class="w-response mx-4 sm:mx-auto mb-5">
  ...
  {#each changelogs as changelog}
    <div class="log-item mb-15">
      <a
        target="_blank"
        href={changelog.file}
        class="log-item-title w-full  flex items-center justify-between sm:justify-start"
      >
        <span>
          {name} APK
          <span class="log-item-tag mr-2">({changelog.tag_name})</span>
        </span>

        <span
          class="leading-normal py-[2px] px-[7px] text-[12px] font-weight-medium rounded-[35px] border border-[currentColor]"
          class:text-[#3fb950]={!changelog.prerelease}
          class:text-[#d29922]={changelog.prerelease}
          >{changelog.prerelease ? 'Pre-release' : 'Release'}</span
        >
      </a>
      ...
      <div class="text-[14px] flex items-center font-weight-medium my-4">
        <img
          ...
          class="w-[25px] h-[25px] rounded-1/2 mr-2"
        />
        <div>
          {changelog.author.login}
          <span class="font-weight-normal">
            {$_('release_at')} {dayjs().to(changelog.published_at)}</span
          >
        </div>
      </div>
        ...
        {#each changelog.assets as asset}
          <li class="">
            <a
              ...
              class="whitespace-pre-wrap flex items-center py-2 px-3 border-b last:border-0 border-gray-500 transition-[background-color] duration-300 ease-in-out hover:bg-[rgba(200,200,200,0.1)] overflow-hidden"
            >
              ...
              <div class="flex-1 flex items-center justify-between">
                <div class="ml-2 ">
                  ...
                  <span
                    class="text-[12px] block mt-1 md:ml-10 md:inline-block md:text-[14px] md:mt-0"
                  >
